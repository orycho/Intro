...
namespace intro {
	...
	llvm::Function *generateCode(const std::vector<intro::Statement *> &statements)
	{
		...
		std::string fun_name = "__AnonFunc";
		...
		llvm::Function *func = llvm::Function::Create(FT, llvm::Function::ExternalLinkage, fun_name, TheModule.get());
	...
	{
		llvm::Function *func = intro::generateCode(statements);
		if (func == nullptr)
		{
		...
		auto ExprSymbol = exitOnError(TheJIT->lookup(func->getName())); // name by agreement with codegen.cxx
	...
	/// Called by parser to execute a single statement in interactive mode
	void executeStatement(Statement *stmt)
	{
		intro::executeStatements(std::vector<intro::Statement *>{stmt});
		initModule(); // reinit LLVM objects to execute another statement
	}
