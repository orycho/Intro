...
extern "C"
{
	...
	void resizeList(rtlist *list, std::uint64_t newsize)
	{
		...
		assert(newmem != NULL && "Memory allocation failure");
	...
	rtstring *allocString(std::uint64_t size)
	{
		...
		assert(result->data != NULL && "Memory allocation failure");
	...
	void resizeString(rtstring *str, std::uint64_t newsize)
	{
		...
		assert(str->data != NULL && "Memory allocation failure");
	...
	void appendString(rtstring *str, const wchar_t *text, std::uint64_t length)
	{
		...
		if (str->size < str->used + length + 1)
		{
			...
			assert(buf != NULL && "Memory allocation failure");
	...
	*/
	...
	static void allocateDictionaryContents(rtdict *dict, std::uint64_t size)
	{
		...
		assert(dict->hashes != NULL && "Memory allocation failure");
	...
	rtdict *allocDictSize(rtt_t key_type, rtt_t value_type, size_t size)
	{
		...
		assert(dict != NULL && "Memory allocation failure");
		initGC(&(dict->gc));
	...
	void setInnerRecord(innerrecord *ir, std::int32_t *offsets, const wchar_t **labels, std::uint32_t fieldcount)
	{
		...
		assert(ir->fieldrtt != NULL && "Memory allocation failure");
		memset(ir->fieldrtt, 0, sizeof(rtt_t) * fieldcount);
		ir->fields = (rtdata *)malloc(sizeof(rtdata) * fieldcount);
		assert(ir->fields != NULL && "Memory allocation failure");
	...
	rtrecord *allocRecord(std::int32_t *offsets, const wchar_t **labels, std::uint32_t fieldcount)
	{
		...
		assert(record != NULL && "Memory allocation failure");
		initGC(&(record->gc));
	...
	rtvariant *allocVariant(std::int32_t *offsets, const wchar_t **labels, std::uint64_t tag, std::uint32_t fieldcount)
	{
		...
		assert(variant != NULL && "Memory allocation failure");
		initGC(&(variant->gc));
		variant->tag = tag;
	...
	rtclosure *allocClosure(std::uint32_t freevarcount)
	{
		...
		assert(closure != NULL && "Memory allocation failure");
		initGC(&(closure->gc));
	...
	rtdata ensureGenerator(rtdata data, rtt_t type)
	{
		...
		assert(retval.ptr != NULL && "Expected a generator subtype");
		return retval;
	}
